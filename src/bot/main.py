...
from bot.handlers.form import handle_form_create, handle_form_fill
...
@safe_handler
async def route_command(client: LolkaClient, channel_id: str, text: str) -> dict:
    ...
    return await client.send_channel_message(channel_id, "Неизвестная команда. Используйте /start")
...
async def run() -> None:
    ...
    async with LolkaClient(settings.lolka_token, settings.lolka_api_base_url) as client:
        ...
        async for event in client.receive_interactive_events():
            ...
            text = event.get("text", "")
            await route_command(client, channel_id, text)
